...
// g++ main_batchsize2_4_8.cpp.cpp train.cpp guessing.cpp md5_batchsize2_4_8.cpp -o main -O1
// g++ main_batchsize2_4_8.cpp.cpp train.cpp guessing.cpp md5_batchsize2_4_8.cpp -o main -O2
#include <iostream>
#include <vector>
#include <chrono>
#include <arm_neon.h>
    ...
    while (!q.priority.empty())
    {
        ...
        if (curr_num > 1000000)
        {
            ...
            for (int i = 0; i < aligned_size; i += BATCH_SIZE)
            {
                ...
                if (BATCH_SIZE == 8)
                    MD5Hash_8SIMD(pw, states, BATCH_SIZE);
                if (BATCH_SIZE == 4)
                    MD5Hash_4SIMD(pw, states, BATCH_SIZE);
                if (BATCH_SIZE == 2)
                    MD5Hash_2SIMD(pw, states, BATCH_SIZE);
            }
