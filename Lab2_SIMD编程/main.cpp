#include "PCFG.h"
#include <chrono>
#include <fstream>
#include "md5.h"
#include <iomanip>
#include <arm_neon.h>
using namespace std;
using namespace chrono;

// 编译指令如下
// g++ main.cpp train.cpp guessing.cpp md5.cpp -o main
// g++ main.cpp train.cpp guessing.cpp md5.cpp -o main -O1
// g++ main.cpp train.cpp guessing.cpp md5.cpp -o main -O2
#include <iostream>
#include <vector>
#include <chrono>
#include <arm_neon.h>
#include <cassert>
#include <cstring>

// NEON优化的MD5函数声明
void MD5Hash_SIMD(const string *inputs, uint32_t states[][4], int batch_size);

// int main()
// {
//     double time_hash = 0;
//     double time_guess = 0;
//     double time_train = 0;
//     PriorityQueue q;

//     // 训练阶段（保持原样）
//     auto start_train = system_clock::now();
//     q.m.train("/guessdata/Rockyou-singleLined-full.txt");
//     q.m.order();
//     auto end_train = system_clock::now();
//     time_train = duration_cast<microseconds>(end_train - start_train).count() / 1e6;
//     q.init();

//     cout << "here" << endl;
//     int curr_num = 0;
//     auto start = system_clock::now();
//     int history = 0;
//     cout << q.priority.size() << endl;

//     while (!q.priority.empty())
//     {
//         q.PopNext();
//         q.total_guesses = q.guesses.size();

//         // 进度监控（保持原样）
//         if (q.total_guesses - curr_num >= 100000)
//         {
//             cout << "Guesses generated: " << history + q.total_guesses << endl;
//             curr_num = q.total_guesses;
//             if (history + q.total_guesses > 10000000)
//             {
//                 auto end = system_clock::now();
//                 time_guess = duration_cast<microseconds>(end - start).count() / 1e6;
//                 cout << "Guess time:" << time_guess - time_hash << "seconds" << endl;
//                 cout << "Hash time:" << time_hash << "seconds" << endl;
//                 cout << "Train time:" << time_train << "seconds" << endl;
//                 break;
//             }
//         }

//         /*--------------- SIMD优化核心部分 ---------------*/
//         if (curr_num > 1000000)
//         {
//             auto start_hash = system_clock::now();

//             // 内存对齐分配（参考网页4的SIMD内存管理）
//             const int SIMD_BATCH = 4;
//             const int aligned_size = (q.guesses.size() / SIMD_BATCH) * SIMD_BATCH;
//             uint32_t (*hash_results)[4] = (uint32_t (*)[4])aligned_alloc(16, aligned_size * sizeof(uint32_t[4]));

//             // 批量处理对齐数据（参考网页2的向量化循环）
//             for (int i = 0; i < aligned_size; i += SIMD_BATCH)
//             {
//                 alignas(16) string batch_pw[SIMD_BATCH]; // 16字节对齐
//                 MD5_NEON simd_states;

//                 // 加载4个密码到寄存器（参考网页3的SIMD数据加载）
//                 for (int j = 0; j < SIMD_BATCH; ++j)
//                 {
//                     batch_pw[j] = q.guesses[i + j];
//                 }

//                 // 调用SIMD哈希函数（参考网页7的MD5核心变换）
//                 MD5Hash_NEON(batch_pw, &simd_states);

//                 // 存储结果（参考网页5的向量存储指令）
//                 alignas(16) uint32_t result_a[4], result_b[4], result_c[4], result_d[4];
//                 vst1q_u32(result_a, simd_states.state[0]);
//                 vst1q_u32(result_b, simd_states.state[1]);
//                 vst1q_u32(result_c, simd_states.state[2]);
//                 vst1q_u32(result_d, simd_states.state[3]);

//                 // 提取每个密码的哈希值
//                 for (int j = 0; j < SIMD_BATCH; ++j)
//                 {
//                     hash_results[i + j][0] = result_a[j];
//                     hash_results[i + j][1] = result_b[j];
//                     hash_results[i + j][2] = result_c[j];
//                     hash_results[i + j][3] = result_d[j];
//                 }
//             }

//             // 处理剩余非对齐数据（回退到串行）
//             for (size_t i = aligned_size; i < q.guesses.size(); ++i)
//             {
//                 uint32_t state[4];
//                 MD5Hash(q.guesses[i], state); // 原始单次处理
//             }

//             free(hash_results);
//             auto end_hash = system_clock::now();
//             time_hash += duration_cast<microseconds>(end_hash - start_hash).count() / 1e6;

//             history += curr_num;
//             curr_num = 0;
//             q.guesses.clear();
//         }
//     }
//     return 0;
// }
int main()
{
    double time_hash = 0;  // 用于MD5哈希的时间
    double time_guess = 0; // 哈希和猜测的总时长
    double time_train = 0; // 模型训练的总时长

    PriorityQueue q;
    auto start_train = system_clock::now();
    q.m.train("/guessdata/Rockyou-singleLined-full.txt");
    q.m.order();
    auto end_train = system_clock::now();
    auto duration_train = duration_cast<microseconds>(end_train - start_train);
    time_train = double(duration_train.count()) * microseconds::period::num / microseconds::period::den;

    q.init();
    cout << "here" << endl;
    int curr_num = 0;
    auto start = system_clock::now();
    // 由于需要定期清空内存，我们在这里记录已生成的猜测总数
    int history = 0;

    // std::ofstream a("./files/results.txt");
    // cout << q.guesses.size() << endl;
    // cout << q.priority.size() << endl;
    while (!q.priority.empty())
    {
        // cout << "q.priority.size(): " << q.priority.size() << ", q.guesses.size(): " << q.guesses.size() << endl;
        q.PopNext();
        q.total_guesses = q.guesses.size();
        // cout << "q.total_guesses: " << q.total_guesses << ", curr_num: " << curr_num << endl;
        if (q.total_guesses - curr_num >= 100000)
        {
            cout << "Guesses generated: " << history + q.total_guesses << endl;
            curr_num = q.total_guesses;

            // 在此处更改实验生成的猜测上限
            int generate_n = 10000000;
            if (history + q.total_guesses > 10000000)
            {
                auto end = system_clock::now();
                auto duration = duration_cast<microseconds>(end - start);
                time_guess = double(duration.count()) * microseconds::period::num / microseconds::period::den;
                cout << "Guess time:" << time_guess - time_hash << "seconds" << endl;
                cout << "Hash time:" << time_hash << "seconds" << endl;
                cout << "Train time:" << time_train << "seconds" << endl;
                break;
            }
        }
        // 为了避免内存超限，我们在q.guesses中口令达到一定数目时，将其中的所有口令取出并且进行哈希
        // 然后，q.guesses将会被清空。为了有效记录已经生成的口令总数，维护一个history变量来进行记录
        if (curr_num > 1000000)
        {
            const int BATCH_SIZE = 8;
            string pw[BATCH_SIZE];
            const int aligned_size = (q.guesses.size() / BATCH_SIZE) * BATCH_SIZE;
            // cout << q.guesses.size() << endl;
            alignas(16) uint32_t states[BATCH_SIZE][4];
            auto start_hash = system_clock::now();

            for (int i = 0; i < aligned_size; i += BATCH_SIZE)
            {
                pw[0] = q.guesses[i];
                pw[1] = q.guesses[i + 1];
                pw[2] = q.guesses[i + 2];
                pw[3] = q.guesses[i + 3];
                pw[4] = q.guesses[i + 4];
                pw[5] = q.guesses[i + 5];
                pw[6] = q.guesses[i + 6];
                pw[7] = q.guesses[i + 7];
                MD5Hash_8NEON(pw, states, BATCH_SIZE);
            }

            // 处理剩余元素
            for (int i = aligned_size; i < q.guesses.size(); ++i)
            {
                // cout << q.guesses.size() - aligned_size << endl;
                bit32 state[4];
                MD5Hash(q.guesses[i], state); // 原始单次处理
            }

            //  在这里对哈希所需的总时长进行计算
            auto end_hash = system_clock::now();
            auto duration = duration_cast<microseconds>(end_hash - start_hash);
            time_hash += double(duration.count()) * microseconds::period::num / microseconds::period::den;

            // 记录已经生成的口令总数
            history += curr_num;
            curr_num = 0;
            q.guesses.clear();
            // cout << "After clear: history = " << history << ", curr_num = " << curr_num << endl;
        }
    }
}
//         if (curr_num > 1000000)
//         {
//             auto start_hash = system_clock::now();
//             bit32 state[4];
//             for (string pw : q.guesses)
//             {
//                 // TODO：对于SIMD实验，将这里替换成你的SIMD MD5函数
//                 MD5Hash(pw, state);

//                 // 以下注释部分用于输出猜测和哈希，但是由于自动测试系统不太能写文件，所以这里你可以改成cout
//                 // a<<pw<<"\t";
//                 // for (int i1 = 0; i1 < 4; i1 += 1)
//                 // {
//                 //     a << std::setw(8) << std::setfill('0') << hex << state[i1];
//                 // }
//                 // a << endl;
//             }

//             //  在这里对哈希所需的总时长进行计算
//             auto end_hash = system_clock::now();
//             auto duration = duration_cast<microseconds>(end_hash - start_hash);
//             time_hash += double(duration.count()) * microseconds::period::num / microseconds::period::den;

//             // 记录已经生成的口令总数
//             history += curr_num;
//             curr_num = 0;
//             q.guesses.clear();
//             cout << "After clear: history = " << history << ", curr_num = " << curr_num << endl;
//         }
//     }
// }