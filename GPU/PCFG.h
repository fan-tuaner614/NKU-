...
public:
    ...
    void Generate(PT pt);
    void BatchGenerate(const std::vector<PT>& pts);
    void BatchGenerateAsync(const std::vector<PT>& pts, int batch_size);
    // 将优先队列最前面的一个PT
    void PopNext();
...
#ifdef __CUDACC__
    // GPU内存管理成员变量（用于统一管理与静态复用）
    char *d_all_data = nullptr;
    char *d_output = nullptr;
    int *d_offsets = nullptr;
    size_t all_data_capacity = 0;
